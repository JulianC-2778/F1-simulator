#!/usr/bin/env python3
"""Validate a real-experiment CSV against its schema (bot_test_plan.md
9.5-9.6, mirroring commentary_test_plan.md 6.2). Used for
ground_truth_strategy / detected_strategy / lap_performance /
control_latency / stability_run / fault_recovery files alike -- pick the
schema with --kind.

Usage:
    python validate_experiment_data.py --kind ground_truth_strategy --file path.csv
    python validate_experiment_data.py --kind detected_strategy --file path.csv
...
from csv_schemas import SCHEMAS, KNOWN_STRATEGIES, CsvSchema  # noqa: E402
...
def validate_rows(rows: list[dict], schema: CsvSchema) -> list[str]:
    ...
    for i, row in enumerate(rows, start=2):  # header is line 1
        ...
        if schema.strategy_column:
            strategy = row.get(schema.strategy_column)
            if strategy and strategy not in KNOWN_STRATEGIES:
                errors.append(
                    f"row {i}: unrecognised strategy {strategy!r} in {schema.strategy_column!r} "
