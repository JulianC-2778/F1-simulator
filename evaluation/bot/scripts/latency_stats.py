...
Verbatim copy of evaluation/commentary/scripts/latency_stats.py -- this
helper has no commentary-specific content, kept in sync deliberately rather
than imported cross-package so evaluation/bot/ stays self-contained.

P95 definition (documented once, used everywhere so results are
reproducible -- bot_test_plan.md 6.4 inherits commentary_test_plan.md 7.3's
requirement that this be pinned down, not left to whatever a library
defaults to): **linear interpolation on the sorted sample**, i.e. numpy's
default ``numpy.percentile(..., method="linear")`` / Excel's
``PERCENTILE.INC``:
